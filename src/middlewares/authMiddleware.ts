...
export const jwtMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  const setNewTokens = (userId: string) => {
    ...
    res.cookie("accessToken", newAccessToken, { httpOnly: true, maxAge: 15 * 60 * 1000, domain: config.COOKIE_DOMAIN });
    res.cookie("refreshToken", newRefreshToken, { httpOnly: true, maxAge: 90 * 24 * 60 * 60 * 1000, domain: config.COOKIE_DOMAIN });

    jwt.verify(newAccessToken, config.ACCESS_TOKEN_SECRET!, (err, decoded) =>
